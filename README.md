...
## Quick start — Hindi step-by-step (एकदम आसान)

1) Poora project folder open karo (terminal ya PowerShell mein):

```bash
cd C:/Users/ronak/Downloads/Travel_devops
```

2) Recommended: Docker se sab kuch ek sath chalana (sabse aasaan):

```bash
# Pehli baar: images build karne ke liye
docker-compose up --build -d

# Stack ko rokne ke liye
docker-compose down

# Agar kisi purane container name conflict aaye to (example):
docker rm -f travel_db travel_backend travel_frontend || true
```

3) Agar Docker nahin chahiye to manually chalana (development):
...
- DB (Postgres): localhost:5432 (user/password jars `.env` file mein defined)

5) Common problems:
- Agar port conflict aaye to pehle check karo kaun process use kar raha hai (`docker ps`, `netstat -a -b`)
- Agar DB se connect nahi hota to `.env` file check karo (DB_USER, DB_PASSWORD, DB_NAME, DB_PORT)

Ab tum khud next time yahi commands use karke project start kar sakte ho.
